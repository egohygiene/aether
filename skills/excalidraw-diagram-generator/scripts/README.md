...
1. **Download and place library file**:
...
1. **Extracts** each icon from the `libraryItems` array
1. **Sanitizes** icon names to create valid filenames (spaces → hyphens, removes special characters)
1. **Saves** each icon as a separate JSON file in the `icons/` directory
1. **Generates** a `reference.md` file with a table mapping icon names to filenames
...
1. Run this script on each library file
1. Move the generated folders to `../libraries/`
1. The AI assistant will use `reference.md` files to locate and use icons efficiently
...
1. **Calculates** the icon’s bounding box
1. **Offsets** all coordinates to the target position
1. **Generates** unique IDs for all elements and groups
1. **Appends** the transformed elements to the diagram
1. **(Optional)** Adds a label beneath the icon
...
1. **(Optional)** Adds a label near the arrow midpoint
1. **Appends** elements to the diagram
1. **Saves** the updated file
