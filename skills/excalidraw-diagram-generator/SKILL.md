...
1. **Key elements** (entities, steps, concepts)
1. **Relationships** (flow, connections, hierarchy)
1. **Complexity** (number of elements)
...
1. **Font**: Always use `fontFamily: 5` (Excalifont) for all text elements
1. **Arrow style**: Use straight arrows for simple flows, curved for complex relationships
...
1. Create flowchart with 4 rectangles + 3 arrows
1. Save as `user-registration-flow.excalidraw`
...
1. Relationships: User → Post ("creates"), User → Comment ("writes"), Post → Comment ("contains")
1. Save as `user-content-relationships.excalidraw`
...
1. Branches: Supervised Learning, Unsupervised Learning, Reinforcement Learning, Deep Learning
1. Sub-topics under each branch
1. Save as `machine-learning-mindmap.excalidraw`
...
1. 📝 Element count
1. 💡 Instructions for opening/editing
...
1. **If library does NOT exist**: Respond with setup instructions:
...
1. **Load specific icon data** (WARNING: Large files):
...
1. **Extract and transform elements**:
...
1. **Position icons and add connections**:
...
1. Read reference.md → Find entries for Internet-gateway, VPC, ELB, EC2, RDS
1. Load:
...
1. Extract elements from each JSON
1. Calculate bounding boxes and offsets for each icon
1. Transform all coordinates (x, y) for positioning
1. Generate unique IDs for all elements
1. Add arrows showing data flow
1. Add text labels
1. Generate final `.excalidraw` file
