...
1. **Use the `&` operator (PowerShell)** - Invoke the magick executable using `&` in PowerShell
1. **Store the path in a variable (PowerShell)** - Assign the ImageMagick path to `$magick` for cleaner code
1. **Wrap in loops** - When processing multiple files, use `ForEach-Object` (PowerShell) or `for` loops (Bash)
1. **Verify dimensions first** - Check image dimensions before processing to avoid unnecessary operations
1. **Use appropriate resize flags** - Consider using `!` to force exact dimensions or `^` for minimum dimensions
