...
1. **Identify file type** - Determine if the source is a prompt (.prompt.md), agent (.agent.md),
instruction (.instructions.md), collection (.collections.md), or MCP server documentation
1. **Extract key examples** - You MUST identify the most common and useful patterns, commands, or use
cases from the source
1. **Follow tldr format strictly** - You MUST use the template structure with proper markdown
formatting
1. **Provide actionable examples** - You MUST include concrete usage examples with correct invocation
syntax for the file type
1. **Adapt to chat context** - Recognize whether you're in inline chat (Ctrl+I) or chat view and
...
1. **Analyze Content**: Extract the file's/documentation's purpose, key parameters, and primary use
cases
1. **Generate tldr**: Create summary using the template format below with correct invocation syntax
for file type
1. **Format Output**:
