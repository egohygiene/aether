...
1. **Hook above the fold**: The first 2-3 lines must compel the reader to click "see more." Front-load value.
1. **Short paragraphs**: 1-3 sentences max per paragraph. Wall of text kills engagement.
1. **Bold sparingly**: Bold key phrases and headers, not entire paragraphs.
1. **Italic for nuance**: Use italic for technical terms, internal thoughts, or subtle emphasis.
1. **Hashtags at the end**: 5-8 relevant hashtags on the last line. No mid-post hashtags.
1. **No emojis in body** unless the user explicitly requests them. Exception: one strategic emoji in CTA (♻️ for repost).
1. **Character limit**: LinkedIn posts can be up to 3000 characters. Aim for 1500-2500 for optimal engagement.
1. **No URLs in body**: LinkedIn suppresses reach for posts with links. Add links in comments instead. Mention "link in comments" or "grab it below" as CTA.
...
1. Identify the best post structure pattern (Hook→Content→CTA, Listicle, Story→Lesson, Resource Share).
1. Extract the core message and 3-5 key points.
1. Apply Unicode bold/italic formatting to headers and emphasis words using `references/unicode-charmap.md`.
1. Add visual separators between sections.
1. Write a compelling hook for the opening.
1. Add a CTA and hashtags at the end.
1. Verify the post is copy-paste ready for LinkedIn.
