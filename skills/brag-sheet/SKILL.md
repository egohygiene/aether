...
1. **DO** check which tools are available (`save_to_brag_sheet`, `git`, `gh`) before choosing a workflow.
1. **DO** always include all three parts: action → result → evidence. If evidence is missing, write `(evidence needed)` — never silently omit.
1. **DO** show drafted entries to the user before saving. Never auto-save without confirmation.
1. **DO** group related commits into a single entry. Ten commits on the same feature = one entry.
1. **DO** preserve the user's voice. Reframe for impact, but don't invent accomplishments or inflate scope.
1. **DO NOT** fabricate metrics, team sizes, or impact numbers. If the user doesn't provide a number, don't invent one.
1. **DO NOT** write entries for work the user only described verbally without verifying. Ask: "Did this ship? Is there a PR or doc I can reference?"
1. **DO NOT** skip the backfill scan steps or draft entries before scanning is complete.
1. **DO NOT** pad weak periods with trivial entries. An honest gap is better than inflated fluff.
...
1. **If git or gh CLI is available** → backfill from commits and PRs (see Backfill section below)

1. **Otherwise** → guided interview: "What did you work on?", "Who benefited?", "What's the evidence?"
...
1. **Select** — pick the top 3–5 highest-impact items
1. **Rewrite** each item with three parts:
...
1. **Organize** by impact theme (not chronologically):
   - Delivering results / operational excellence
   - Customer / team impact
   - Collaboration / mentoring / leadership
   - Growth / learning
1. **Ask for gaps** — if evidence is missing, prompt the user: "What metric changed?", "Who was unblocked?", "What's the PR or incident ID?"
...
1. No fabricated metrics — only user-provided or source-verified data
1. Entries shown to user before saving
1. Time range explicitly stated
1. Output is pasteable markdown with categories assigned
...
1. Check `gh pr list --author @me --state merged` for cross-repo PRs
1. Fall back to the guided interview — not all impactful work leaves git traces (design docs, incident response, mentoring)
