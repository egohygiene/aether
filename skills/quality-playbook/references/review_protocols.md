...
1. Read the test: identify which function it calls and what condition it asserts
1. Confirm alignment: the test must call the function cited in the finding, trigger the specific condition the finding describes, and assert on the behavior the finding says is wrong
...
1. **Count test functions in `quality/test_regression.*`.** This should equal or exceed the BUG count (some BUGs may need multiple tests).
1. **For each BUG row in the summary**, verify it has either:
   - A `REGRESSION TEST:` line citing the test function name, OR
   - An `EXEMPTION:` line explaining why no test was written
1. **If any BUG lacks both**, go back and write the test or the exemption before declaring the review complete.
...
1. For each, write a regression test in `quality/test_regression.*` using the same format as code review regression tests. Use the spec audit report as the source citation: `[BUG from spec_audits/YYYY-MM-DD-triage.md]`.
1. Run the test to confirm it fails (expected) or passes (needs investigation).
1. Update the cumulative BUG tracker in PROGRESS.md with the test reference.
...
1. **Name the test file** `quality/test_regression.*` using the project's language:
...
1. **Each test should document its origin:**
...
1. **Run the tests and report results** as a confirmation table:
...
1. **If a test passes unexpectedly**, investigate — either the finding was a false positive, or the test doesn't exercise the right code path. Report as NEEDS INVESTIGATION, not as a confirmed bug.
...
1. **Reading bodies** prevents the common failure of assuming a function works based on its name
1. **QUESTION vs BUG** reduces false positives that waste human time
1. **Grep before claiming missing** prevents the most common AI review hallucination: claiming something doesn't exist when it's in a different file
