...
1. If `quality/spec_audits/` exists but contains no triage summary, the spec audit did not run.
1. If `quality/test_regression.*` exists but contains only imports and no test functions, regression tests were not written.
