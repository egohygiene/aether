...
1. If it exists: read all events. Find the last `phase_start` not followed by a matching `phase_end`. Call it the "in-progress phase".
1. Verify the in-progress phase's expected artifacts (per cross-validation rules above):
...
1. If all 6 `phase_end` events are present but no `run_end`: append `run_end status=success` and finalize.
...
1. `_index` is line 1.
1. Every line is valid JSON (one object per line).
1. Every event has `ts` and `event` fields.
1. Every `event` value appears in `_index.event_types`.
1. Append-only: events are added, never edited. Editing a prior event is a schema violation.
1. `phase_start` and `phase_end` events for a given phase appear at most once per run (no out-of-order or duplicate phase markers).
1. `run_start` is the second line (after `_index`); `run_end` is the last line if the run completed.
