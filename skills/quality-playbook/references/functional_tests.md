...
1. **Read the documentation** — docstrings, Javadoc, TSDoc, ScalaDoc. They often specify return types, exceptions, and edge case behavior.
1. **Read one existing test that calls it** — existing tests show you the exact calling convention, fixture shape, and assertion pattern.
1. **Read real data files** — if the function processes configs, schemas, or data files, read an actual file from the project. Your test fixtures must match this shape exactly.
...
1. **Execute** — Call the function, run the pipeline, make the request
1. **Assert specific properties** the spec requires
...
1. Process the mutated input
1. Assert graceful handling — the result is valid despite the edge-case input
