...
1. **Ask about scope.** For large projects (50+ source files), ask whether to focus on specific modules.
...
1. **parity** — Compare parallel code paths
1. **adversarial** — Challenge prior dismissals, recover Type II errors
...
1. **Phase 2 (Generate)** — Requirements, constitution, tests, protocols → artifact set in `quality/`
1. **Phase 3 (Code Review)** — Three-pass review, regression tests → `quality/code_reviews/`, patches
1. **Phase 4 (Spec Audit)** — Three auditors, triage with probes → `quality/spec_audits/`
1. **Phase 5 (Reconciliation)** — TDD red-green verification → `quality/BUGS.md`, TDD logs
1. **Phase 6 (Verify)** — 45 self-check benchmarks → final PROGRESS.md checkpoint
