...
1. quality/EXPLORATION.md - Phase 1 findings (especially the "Candidate Bugs for Phase 2" section)
1. quality/REQUIREMENTS.md - derived requirements and use cases
1. quality/CONTRACTS.md - behavioral contracts
1. SKILL.md - read the Phase 3 section ("Phase 3: Code Review and Regression Tests"). Also read references/review_protocols.md. Resolve SKILL.md and the references/ directory via the documented fallback list above; do NOT assume any single install layout.
...
1. For every grid, I applied the BUG-default rule mechanically.
1. Every BUG emitted for a pattern-tagged REQ has a `- Covers: [...]` field with valid cell IDs.
1. Every BUG whose Covers list has ≥2 entries has a non-empty `- Consolidation rationale: ...` field.
1. For every downgraded cell, I wrote a complete structured record in `quality/compensation_grid_downgrades.json` with all five required fields and a valid `reason_class`.
1. For every pattern-tagged REQ, the union of Covers lists + downgrade cells equals the grid's cell set.
