...
1. A diagnosis of the likely root cause
1. A proposed fix shape (do NOT apply it)
...
1. `## Quality Risks` — domain-knowledge risk analysis. Numbered or
   bulleted; cite file:line where risks are concretely visible in
   code or docs.

1. `## Pattern Applicability Matrix` — a Markdown table with one row
...
1. `## Pattern Deep Dive — <pattern-name>` — at least 3 sections,
...
1. `## Candidate Bugs for Phase 2` — numbered list of bug
...
1. `## Gate Self-Check` — proves you ran the Phase 1 gate. List each
...
1. For every REQ that passed Gate 1, I ran Gate 2 (function-level similarity).
1. Where both gates passed, I emitted per-site UCs (UC-N.a, UC-N.b, …).
1. Where only Gate 1 passed, I marked the cluster `<!-- cluster: heterogeneous -->`.
1. Where neither gate passed, I kept a single umbrella UC without marking.
1. For each REQ with a pattern match in Gate 1, I added `Pattern: whitelist|parity|compensation` to the REQ block.
