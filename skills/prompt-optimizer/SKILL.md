...
1. **Identify the audience and use.** Who reads the output, and what will they do with it? This drives tone and format.
1. **Decide: Case A or Case B.** Did the user provide the actual content, or just describe a class of task? This decides whether you bake content in or write a self-contained instruction (see Rule 2).
1. **Spot the gaps.** Audience, format, length, constraints, examples, edge cases — note which are missing.
1. **Handle the gaps correctly.** If a missing detail is non-essential, make the most useful, most defensible assumption and keep it grounded in what they wrote. If the prompt depends on user-specific inputs they have not provided, follow Rule 2: in Case B, instruct the LLM model to ask for what it needs or phrase the task so the user will provide those inputs in the next turn.
1. **Pick a structure.** Single-paragraph instruction for simple tasks. XML tags for anything with multiple sections.
1. **Write the prompt.** Apply the principles below.
1. **End with the closing line.**
1. **Scan for brackets.** Before you finalize, re-read your output looking for `[`, `{`, or `<...your...>`-style placeholders. Kill any you find.
