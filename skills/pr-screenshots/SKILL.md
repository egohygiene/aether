...
1. **Keep descriptions brief** — a sentence or two per image pointing out what changed is enough
1. **Prefer visible images over collapsed sections** — screenshots behind `<details>` tags are easy to skip
1. **Annotate when the change is subtle** — use the `image-annotations` skill to add callouts when the difference isn't immediately obvious
1. **Match viewport and crop** between before/after pairs so the comparison is meaningful
