...
1. **Contrast scoring**: prefers placements where label text is readable — `abs(avg_brightness - 147) - std * 0.3 - dist * 0.02`
1. **Joint resolution**: candidates computed independently, placed greedily (best score first)
1. **Hard blocks**: labels cannot overlap any other annotation's element or breathing box
1. **Proximity penalty**: labels within 40px of other placed boxes get a score penalty
1. **Arrow crossing penalty**: -50 for arrows crossing already-placed arrows
...
1. **Labels go RIGHT NEXT to what they describe** — short arrow (30-80px), label adjacent. Viewer's eye shouldn't travel more than ~100px
1. **Arrow must not cross its own label** — pick the edge closest to the target
1. **No bottom bar / subtitle approach** — eyes jump between content and bar. Contextual placement only
1. **Hero message gets a bigger font** — main takeaway 64pt+, detail annotations 38pt

### Timing and pacing

1. **Fade: 2-frame pop-in at 10fps** — 50% → 100% opacity (0.2s total). Easing curves look bad at low FPS
1. **Type → pause → annotate** — during fast action, show NO annotation. Pause, then add it
1. **Variable frame duration** — fast during action (100ms), slow during pauses (600-800ms), long hold for hero (500ms)
1. **Higher FPS for smooth motion** — 10fps minimum for typing/interaction
...
1. Place labels **close to the rect** — short leader line (25-35px)
1. Labels can overlap content — the stroke gives enough contrast
1. **Show locally first** — verify before uploading to a PR
1. **Take screenshots at native 1x, control display size in HTML** — use `<img width="300">` in markdown, never resize with PIL (creates artifacts)
1. **Always check `Image.open(path).size` first** — HiDPI screenshots are larger than they appear (150% scaling = 1.5x CSS pixel dimensions)
1. **Short labels work better** — wide labels have fewer valid placements. Use 1-3 words when possible
1. **Verify with debug=True** — always check the first annotation of a new image with debug mode
