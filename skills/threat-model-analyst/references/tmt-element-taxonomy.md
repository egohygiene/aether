...
1. **Services/Logic** → Processes
   - Business logic classes, service layers, workers

1. **Data Access** → Data Stores + flows
   - Repository classes, DB contexts, cache clients

1. **External Calls** → External Interactors + outbound flows
   - HTTP clients, SDK integrations, third-party APIs

1. **Security Boundaries** → Trust Boundaries
   - Auth middleware, network segments, deployment units

1. **Kubernetes Pod Composition** → Sidecar co-location
