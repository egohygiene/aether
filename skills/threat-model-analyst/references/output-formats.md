...
1. Derive all content from code analysis — do not speculate
1. If a section cannot be determined, state that explicitly
1. Target: **150-250 lines** minimum. Previous iterations produced only 100-150 lines which is too thin. Include detailed component descriptions, port/protocol info, and substantial scenario narratives.
1. Key Components table should align with threat model diagram elements
1. Use **architecture** diagram styles (not DFD) — see `diagram-conventions.md`
1. After writing, verify each Mermaid block has valid syntax
1. **Top Scenarios**: The first 3 scenarios MUST include Mermaid `sequenceDiagram` blocks showing the interaction flow. Each sequence diagram should show actual participants, messages with protocol details, and alt/opt blocks for error paths.
1. **Component alignment**: Every component listed in Key Components MUST later appear as a section in `2-stride-analysis.md`
1. **Deployment Model**: Must include specific details: ports, protocols, bind addresses, network exposure, and deployment topology (single machine / cluster / multi-tier)
1. **Security Infrastructure Inventory**: Populate with EVERY security-relevant component found in code (auth, encryption, access control, logging, secrets management)
...
1. Summary table **MUST include T1, T2, T3 columns**
1. All three tier sub-sections appear for every component (even if empty — use "*No Tier N threats identified*")
...
1. **Threat coverage check**: Every threat ID in `2-stride-analysis.md` must be referenced by at least one finding
1. **Sort order check**: Within each tier, no higher-CVSS finding appears after a lower-CVSS finding in the same severity band
1. **CVSS-to-Tier consistency**: Scan every finding — if CVSS has `AV:L` or `PR:H`, finding MUST NOT be in Tier 1. Fix by downgrading the tier, not by changing the CVSS.
1. **Threat Coverage Verification table**: At the end of `3-findings.md`, include:
...
1. If Platform count > 20% of total threats → **you are overusing Platform as an escape hatch. Re-examine.**
1. If any threat is listed as `⚠️ Accepted Risk` or `⚠️ Needs Review` → **VIOLATION. Create a finding or verify it's genuinely Platform.**
...
1. **Executive Summary** (REQUIRED) — Risk rating + coverage. NO separate "Key Recommendations" subsection
1. **Action Summary** (REQUIRED) — Tier-based prioritized action plan with `### Quick Wins` subsection
1. **Analysis Context & Assumptions** (REQUIRED) — Scope, infrastructure context, `### Needs Verification` table, finding overrides
1. **References Consulted** (REQUIRED) — Security standards + component documentation
1. **Report Metadata** (REQUIRED) — Model, timestamps, duration, git info
1. **Classification Reference** (REQUIRED) — MUST be the last section. Static table copied from skeleton.
...
1. Quick Wins lists only Tier 1 findings with `Remediation Effort: Low` — highest-impact, lowest-effort items
1. If no Tier 1 Low-effort findings exist, show Tier 2 Low-effort findings instead, with a note: "No Tier 1 quick wins identified. These Tier 2 items offer the best effort-to-impact ratio:"
1. If no Low-effort findings exist at all, keep `### Quick Wins` heading and add: `No low-effort findings identified. All findings require Medium or High effort.`
1. Verify: Findings column sums must equal total findings count in `3-findings.md`
1. Verify: Threats column sums must equal total threats count in `2-stride-analysis.md` summary table
...
1. Every row MUST have a full URL (https://...) — never omit the URL column
1. Populate Component Documentation with technologies actually consulted during analysis
1. Do not add documentation that was not used
...
1. Report Metadata values all wrapped in backticks
1. Finding Overrides always uses table format (even when empty)
1. Report Files section always first
1. `0.1-architecture.md` always listed in Report Files table
...
1. ❌ Flat STRIDE tables → ✅ Split into Tier 1/2/3 sub-sections per component
1. ❌ Missing `Exploitability Tier` and `Remediation Effort` → ✅ MANDATORY on every finding
1. ❌ STRIDE summary missing T1/T2/T3 columns → ✅ Include T1|T2|T3 columns
1. ❌ Wrapping `.md` in ` ```markdown ` code fences → ✅ Start with `# Heading` on line 1. The `create_file` tool writes raw content — fences become literal text in the file.
1. ❌ Wrapping `.mmd` in ` ```plaintext ` or ` ```mermaid ` → ✅ Start with `%%{init:` on line 1. The `.mmd` file is raw Mermaid source.
1. ❌ Missing Action Summary → ✅ Section MUST be titled exactly `## Action Summary`. MUST include `### Quick Wins` subsection with table of Tier 1 low-effort findings.
1. ❌ Missing threat count context paragraph → ✅ Include `> **Note on threat counts:**` blockquote in Executive Summary
1. ❌ Omitting empty tier sections → ✅ Always include all three tiers per component
1. ❌ Adding separate `### Key Recommendations` or `### Top Recommendations` or `### Priority Remediation Roadmap` → ✅ Action Summary IS the recommendations — no other name.
1. ❌ Drawing sidecars as separate nodes → ✅ See `diagram-conventions.md` Rule 1
1. ❌ Missing CVSS 4.0 vector string → ✅ Every finding MUST have both score AND full vector (e.g., `CVSS:4.0/AV:N/AC:L/...`)
1. ❌ Missing CWE or OWASP on findings → ✅ MANDATORY on every finding
1. ❌ Using OWASP `:2021` suffix → ✅ ALWAYS use `:2025` (e.g., `A01:2025 – Broken Access Control`). The 2025 edition is current.
1. ❌ Missing Threat Coverage Verification table → ✅ Required at end of `3-findings.md`
1. ❌ Architecture component not in STRIDE analysis → ✅ Every component in 0.1-architecture.md must have a STRIDE section
1. ❌ Missing sequence diagrams for top scenarios → ✅ First 3 scenarios in 0.1-architecture.md MUST have Mermaid sequence diagrams
1. ❌ Missing Needs Verification section in 0-assessment.md → ✅ Include under Analysis Context & Assumptions
1. ❌ Missing `## Analysis Context & Assumptions` section entirely → ✅ REQUIRED. Previous iterations skipped this section. Must include Scope, Needs Verification, and Finding Overrides sub-tables.
1. ❌ Missing `### Quick Wins` subsection → ✅ REQUIRED under Action Summary. List Tier 1 low-effort findings; if none, include heading with note.
1. ❌ Skipping `## Report Files`, `## References Consulted`, or `## Report Metadata` → ✅ ALL 7 sections in 0-assessment.md are MANDATORY. Never omit any.
1. ❌ Finding IDs out of order (FIND-06 before FIND-04) → ✅ Finding IDs MUST be sequential top-to-bottom: FIND-01, FIND-02, FIND-03, ... Renumber after sorting.
1. ❌ CWE without hyperlink → ✅ CWE MUST include hyperlink: `[CWE-306](https://cwe.mitre.org/data/definitions/306.html): Missing Authentication for Critical Function`
1. ❌ Time estimates or scheduling in output → ✅ NEVER generate `~1 hour`, `Sprint 1-2`, `Phase 1 — Immediate`, `(hours)`, or any timeline/duration in ANY output file. The report says WHAT to fix, not WHEN.
...
1. Populate from the same analysis data used to write the markdown files
1. Ensure component IDs use PascalCase derived from actual class/file names
1. Ensure flow IDs use the canonical `DF_{Source}_to_{Target}` format
1. All threat and finding identity keys must reference actual code artifacts (file paths, config keys)
1. Include git metadata from Step 1 (commit, branch, date)
1. The `metrics` object must match the counts in the markdown reports
1. This file is NOT listed in the Report Files table of `0-assessment.md`
1. Populate `aliases`, `boundary_kind`/`kind`, `fingerprint`, and `contains_fingerprint` for deterministic matching
1. If a component has multiple observed names in the same run, keep one canonical `id` and store all alternates in `aliases`
