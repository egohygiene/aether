...
1. **Keep individually:** entry points, core flow components, security-critical services, primary data stores, all external actors.
1. **Aggregate only:** supporting infrastructure, secondary caches, multiple externals at same trust level.
1. **Aggregated element labels MUST list contents:**

   ```
   DataLayer[("Data Layer<br/>(UserDB, OrderDB, Redis)")]
   SupportServices(("Supporting<br/>(Logging, Monitoring)"))
   ```
1. **Flow IDs:** Use `SDF` prefix: `SDF01`, `SDF02`, ...
