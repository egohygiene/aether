...
1. A prior `threat-model-*` folder exists in the repository with a valid `threat-inventory.json`
1. The user provides or implies both: a baseline report folder AND a target commit (defaults to HEAD)
...
1. **Baseline code access — reuse or create worktree:**
...
- Tech stack, deployment model: update if changed, otherwise carry forward
  ...
  1. `### Component Exposure Table` with columns: Component, Listens On, Auth Required, Reachability, Min Prerequisite, Derived Tier
...
1. **The `## Summary` table MUST appear at the TOP of the file**, immediately after `## Exploitability Tiers`, BEFORE any individual component sections. Use this EXACT structure at the top:
...
⛔ POST-STEP CHECK: After writing the Change column for ALL threats, verify:
  ...
  1. No old-style tags: Still Present, New (Code), New (Modified), Previously Unidentified
  1. Fixed threats cite the specific code change
...
1. Read the same files at the baseline commit: `cat {BASELINE_WORKTREE}/{file_path}`
1. If the vulnerability pattern exists in the old code → `previously_unidentified`
1. If the vulnerability pattern does NOT exist in the old code → `new_in_modified`
...
1. Report Files
1. Executive Summary (with `> **Note on threat counts:**` blockquote)
1. Action Summary (with `### Quick Wins`)
1. Analysis Context & Assumptions (with `### Needs Verification` and `### Finding Overrides`)
1. References Consulted
1. Report Metadata
1. Classification Reference (static table copied from skeleton)
...
1. For each FAIL → apply the check's "Fail remediation" action
1. Re-run failed checks to confirm they pass
1. After 2 correction attempts, escalate remaining failures to Needs Verification
1. Record end time and generate execution summary
