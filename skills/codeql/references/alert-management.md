...
1. The 75th percentile of CVSS scores for those CVEs is calculated
1. That score becomes the query's security severity
1. The numerical score maps to Critical/High/Medium/Low per CVSS definitions
...
1. Alert information is sent to the LLM for analysis
1. Fix suggestions are posted as PR comments with inline code changes
1. Developers review, edit, and commit the suggested fix
...
1. The next code scanning run will verify the fix
1. Alert is automatically closed when the fix is confirmed
...
1. Identify stale configurations
1. Delete the stale configuration to remove outdated alerts
