...
1. Run `codeql database create` with appropriate build commands
1. Run `codeql database analyze` to generate SARIF
1. Run `codeql github upload-results` to push results to GitHub
1. Set `GITHUB_TOKEN` with `security-events: write` permission
