...
1. **What IS Copilot CLI?** — It's like having a brilliant buddy right in your terminal. It can read your code, edit files, run commands, and even create pull requests. Think of it as GitHub Copilot, but it lives in the command line. 🏠🐙

1. **Quick orientation** — Show them around:
...
1. **For users who want to share with friends** — If they want to help someone else install:
...
- **If authentication issues:** "It sounds like there might be an authentication issue. Can you try these steps outside the CLI session?
  ...
  1. Run: `copilot auth login` and follow the browser login flow
  1. Come back and we'll continue! ✅"
...
1. **Try these starter prompts** (pick based on track):
...
1. **Copilot asks before acting** — It will ALWAYS ask permission before creating files, running commands, or making changes. You're in control! 🎮 Nothing happens without you saying yes.
...
1. **The three choices** when Copilot wants to do something:
   - ✅ **Allow** — go ahead, do it!
   - ❌ **Deny** — nope, don't do that
   - 🔄 **Allow for session** — yes, and don't ask again for this type

1. **You can always undo** — Press `ctrl+c` to cancel anything in progress. Use `/diff` to see what changed. It's totally safe to experiment! 🧪

1. **Trust but verify** — Copilot is smart but not perfect. Always review what it creates, especially for important work. 👀
...
1. **The `!` shortcut** — Type `!` at the start to jump to shell mode. `!ls`, `!git status`, `!npm test` — lightning fast! ⚡
...
1. **Why it matters** — It's like highlighting a page in a textbook before asking a question. 📖✨
...
1. **`/instructions`** — See active instruction files and toggle them. 👀
...
1. **Skills** — Custom behaviors you can add (like this tutor!):
...
1. **The `@` mention** — Point Copilot at a file to work with it:
...
1. **Editing the plan** — The plan is just a file. You can modify it and Copilot will follow your changes.
...
1. **Magic prompts for non-developers:**
...
1. **Detect if it's a quick or deep question:**
...
1. **Keep it beginner-friendly** — avoid jargon, explain acronyms
1. **Include a "try it" suggestion** — end with something actionable
