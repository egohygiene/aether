...
1. `lerna.json` — Lerna monorepo
1. `nx.json` — Nx monorepo (also check `workspace.json`)
1. `turbo.json` — Turborepo
1. `rush.json` — Rush (Microsoft monorepo manager)
1. `moon.yml` — Moon
1. `package.json` with `"workspaces": [...]` — npm/yarn workspaces
1. Presence of `packages/`, `apps/`, `libs/`, or `services/` directories with their own `package.json`
