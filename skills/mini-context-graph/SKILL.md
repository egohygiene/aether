...
1. **Graph layer** — Entities and relations are extracted once and stored as a navigable knowledge graph. BFS traversal answers structural queries without re-reading sources.
1. **Raw source layer** — Original documents are stored immutably with chunks. Provenance links tie every graph node and edge back to the exact text that supports it.
...
1. Read `references/ontology.md` — type normalization rules.
1. Extract entities and relations using your LLM reasoning.
1. Call `skill.ingest_with_content(...)` — stores raw content + chunks + graph nodes + provenance.
1. **Write a wiki summary page** using `wiki_store.write_page(category="summary", ...)`.
1. **Update entity pages** — for each new/updated entity, write or update `wiki_store.write_page(category="entity", ...)`.
1. **Update topic pages** if the document touches an existing synthesis topic.
1. A single document ingest will typically touch 3–10 wiki pages.
...
1. If the wiki has a good answer, synthesize from wiki pages (fast path).
1. If deeper graph traversal is needed, call `skill.query_with_evidence(query)`.
1. Return the answer with evidence citations from `supporting_documents`.
1. If the answer is valuable, file it back as a new wiki topic page.
