...
1. Re-push within **3 hours** (after that, repeat the bypass process)
...
1. Options to remove the secret or bypass are presented inline
1. Same bypass reasons apply as command line
...
1. Users on the bypass list can bypass push protection directly (with a reason)
1. All other contributors must **submit a bypass request** for review
1. Bypass requests appear in the Security tab → "Push protection bypass" page
1. Requests expire after **7 days** if not reviewed
...
1. Enable "Restrict who can bypass push protection"
1. Add users, teams, or roles to the bypass list

**Organization level:**

1. Organization Settings → Advanced Security → Global settings
1. Configure delegated bypass in security configuration
...
1. Review pending requests (includes the secret, commit, and contributor's comment)
1. **Approve** — contributor can push the secret and any future commits with the same secret
1. **Deny** — contributor must remove the secret before pushing

### Bypass Request Flow (Contributor Perspective)

1. Push is blocked; visit the URL from the error message
1. Add a comment explaining why the secret is safe to push
1. Click **Submit request**
1. Wait for email notification of approval/denial
1. If approved: push the commit
1. If denied: remove the secret and push again
...
1. The pattern will now block pushes containing matching secrets
