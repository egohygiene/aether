...
Shell and PowerShell implementations for the Aether Copilot agent hook
baseline.  These scripts are invoked by the hook configuration in
[`../.github/hooks/aether.json`](../../.github/hooks/aether.json).
...
1. Add a new entry to `.github/hooks/aether.json` following the existing
   pattern (`"type": "command"`, explicit `timeoutSec`, both `bash` and
   `powershell` fields, `"cwd": "."`).
1. Create the Bash implementation under `scripts/copilot-hooks/`.
1. Create the PowerShell equivalent.
1. Add test fixtures to `scripts/copilot-hooks/tests/`.
1. Update `.github/hooks/README.md` to document the new hook.
1. Run both validation and test Taskfile commands before committing.
