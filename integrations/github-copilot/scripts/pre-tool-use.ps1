...
#
# pre-tool-use.ps1 — Copilot preToolUse safety guardrail for Aether (PowerShell)
#
...
function Test-DeniedPattern {
  ...
  if ($n -match 'git\s+.*filter-branch') {
    $denyReason = (
      'git filter-branch rewrites history and is not permitted. ' +
      'Use git filter-repo for intentional history surgery.'
    )
    Write-Deny $denyReason
    return $true
  }
  ...
  $credentialFilePattern = (
    '(cat|less|more|head|tail|Get-Content|type)\s+.*' +
    '(\.netrc|\.ssh.id_|\.aws.credentials|\.aws.config|\.env\b|' +
    'credentials\.json|keystore|\.pem|\.p12|\.pfx)'
  )
  if ($n -match $credentialFilePattern) {
