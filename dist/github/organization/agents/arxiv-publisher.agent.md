...
Apply the [`prepare-arxiv-release`](.agents/skills/prepare-arxiv-release/SKILL.md) skill and follow [`specs/publishing/arxiv.spec.md`](specs/publishing/arxiv.spec.md). Treat live arXiv requirements as time-sensitive; verify them from authoritative arXiv documentation when the task depends on current submission rules.
