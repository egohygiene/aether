...
#
# session-start.ps1 — Copilot sessionStart hook for Aether (PowerShell)
#
...
function Test-RepositoryRoot {
  ...
  if (-not (Test-Path 'Taskfile.yml')) {
    $warningMessage = (
      ...
      'session may not be running from the repository root'
    )
    Write-HookLog -Label 'WARN' -Message $warningMessage
    return $false
  }
