...
1. **If one key is set** — use that provider. No need to ask.
1. **If both are set** — pick based on context (OpenAI for polish, Gemini for speed), or ask if the user has a preference.
1. **If no keys are set** — run the Onboarding section.
1. **Generate the image** using the appropriate API reference.
1. **Tell the user** where the image was saved.
...
1. Proceed to generate the image they originally asked for.
