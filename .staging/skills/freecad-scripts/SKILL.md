...
1. **Implicit document**: If no document handling is mentioned, wrap in standard `doc = FreeCAD.ActiveDocument or FreeCAD.newDocument()`
1. **Units assumption**: Default to millimeters unless stated otherwise
1. **Recompute**: Always call `doc.recompute()` after modifications
1. **GUI guard**: Wrap GUI-dependent code in `if FreeCAD.GuiUp:` when the script may run headless
1. **Part.show()**: Use `Part.show(shape, "Name")` for quick display, or `doc.addObject("Part::Feature", "Name")` for named persistent objects
...
1. [geometry-and-shapes.md](references/geometry-and-shapes.md) — Part, Mesh, Sketcher, topology
1. [parametric-objects.md](references/parametric-objects.md) — FeaturePython, properties, scripted objects
1. [gui-and-interface.md](references/gui-and-interface.md) — PySide, dialogs, task panels, Coin3D
1. [workbenches-and-advanced.md](references/workbenches-and-advanced.md) — Workbenches, macros, FEM, Path, recipes
