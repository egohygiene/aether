...
1. **Entities / actors** — What are the main components, actors, classes, or tables?
1. **Relationships** — How are they connected? What direction? What cardinality?
1. **Output path** — Where should the `.drawio` file be saved?
1. **Existing file** — Are we creating new or editing an existing file?
...
1. **Provide a brief description** of what is in the diagram so the user knows what to expect.
...
1. **Identify the target diagram page** — by index or `name` attribute
1. **Assign new unique ids** that do not collide with existing ids
1. **Respect the container hierarchy** — children of a swimlane use coordinates relative to their parent
1. **Verify edges** — after repositioning nodes, confirm edge source/target ids remain valid
...
1. **How to edit it** (if the user is likely to customise):
   > "Click any shape to select it. Double-click to edit the label. Drag to reposition."
1. **Validation status** — whether the validator script was run and passed
