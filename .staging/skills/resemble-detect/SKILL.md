...
1. **Standalone** — `POST /intelligence` with a URL (when you only need analysis, not a deepfake verdict)
...
1. Read `metrics` / `image_metrics` / `video_metrics` for the verdict
1. Read `intelligence.description` for structured media analysis
1. If audio labeled `"fake"`, check `audio_source_tracing.label` for the source platform
1. Ask follow-up questions via Detect Intelligence if anything needs clarification
1. Check for watermarks via `POST /watermark/detect` if provenance is relevant

### Quick Authenticity Check (Fastest)

1. Submit minimal detection: `{ "url": "..." }`
1. Poll until complete
1. Check `label` and `aggregated_score` (audio) or `label` and `score` (image/video)
1. Report result with score context

### Provenance Pipeline (Content Creators)

1. Apply watermark to original content: `POST /watermark/apply`
1. Distribute watermarked media
1. Later, verify provenance: `POST /watermark/detect` against any copy
...
1. **Provide score context** — Use the score interpretation table above
1. **Mention limitations** — Detection is probabilistic, not absolute proof
1. **Include actionable next steps** — Suggest intelligence queries, source tracing, or watermark checks as appropriate
1. **For inconclusive results (0.3–0.5)** — Explicitly state the result is inconclusive and recommend additional analysis with different parameters or manual review
1. **Never present detection as legal evidence** — Detection results are analytical tools, not forensic certifications
