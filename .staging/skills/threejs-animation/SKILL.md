...
1. **AnimationMixer** - Plays animations on a root object
1. **AnimationAction** - Controls playback of a clip
...
1. **Optimize clips**: Call `clip.optimize()` to remove redundant keyframes
1. **Disable when off-screen**: Stop mixer updates for invisible objects
1. **Use LOD for animations**: Simpler rigs for distant characters
1. **Limit active mixers**: Each mixer.update() has a cost
