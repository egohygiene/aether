...
1. View it to see actual pixel positions
1. Crop with PIL based on what you see
1. View the result — if not right, re-crop (instant, no re-screenshot needed)
...
1. **Before/after pairs must use the same viewport width and crop** — otherwise the comparison is useless
1. **To get a "before" after you already changed code**: use `git checkout HEAD~1 -- <files>` to revert, screenshot, then `git checkout HEAD -- <files>` to restore
1. **For interactive states**: capture before AND after for each state — don't assume the "normal" before covers all cases
1. **Use `device_scale_factor=1`** in Playwright to force 1x pixels so screenshots match what users see at 100% zoom
1. **Charts need extra wait time** — Plotly, D3, etc. render asynchronously; 4s minimum after networkidle
1. **Narrow viewport reveals rendering bugs** — some border/alignment issues only appear at specific widths
