...
1. **Is there an explicit list of what tools the agent can call?** (ASI-02)
1. **Can the agent do anything, or are its capabilities bounded?** (ASI-03)
1. **Can the agent promote its own privileges?** (ASI-04)
1. **Do agents verify each other's identity before accepting tasks?** (ASI-05)
1. **Is every tool call logged with enough detail to replay it?** (ASI-06)
1. **Does each agent have a unique cryptographic identity?** (ASI-07)
1. **Is policy enforcement deterministic (not LLM-based)?** (ASI-08)
1. **Are plugins/tools integrity-verified before use?** (ASI-09)
1. **Is there a circuit breaker or kill switch?** (ASI-10)
