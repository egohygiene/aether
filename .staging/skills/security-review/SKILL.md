...
1. **Traces across files** — following how user input moves through your application
1. **Self-verifies findings** — re-examines each result to filter false positives
1. **Assigns severity ratings** — CRITICAL / HIGH / MEDIUM / LOW / INFO
1. **Proposes targeted patches** — every finding includes a concrete fix
1. **Requires human approval** — nothing is auto-applied; you always review first
...
1. Ask: "Is this actually exploitable, or is there sanitization I missed?"
1. Check if a framework or middleware already handles this upstream
1. Downgrade or discard findings that aren't genuine vulnerabilities
1. Assign final severity: CRITICAL / HIGH / MEDIUM / LOW / INFO
