...
1. Has been deprecated by its maintainer with a security advisory
1. Is a fork of a known package from an unknown publisher (typosquatting)
1. Has a name that's one character off from a popular package (e.g., `lodash` vs `1odash`)
1. Was recently transferred to a new owner (check git history / npm transfer notices)
