...
1. **Pipeline** — `explore` → understand → `general-purpose` → implement → `code-review` → verify
1. **Specialist handoff** — Identify task → `/agent` to pick specialist → review with `/fleet` or `/tasks`
