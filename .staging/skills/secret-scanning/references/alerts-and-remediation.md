...
1. GitHub periodically sends the secret to the issuer's API
1. Validation result is displayed on the alert
...
1. **Assess severity** — check validity status (active = urgent)
1. **Rotate the credential** — revoke the old credential and generate a new one
1. **Update references** — update all code/config that used the old credential
1. **Investigate impact** — check logs for unauthorized use during the exposure window
1. **Close the alert** — mark as resolved with appropriate reason
1. **Optionally clean Git history** — remove from commit history (time-intensive)
