...
1. [Branch Protection Rules](#2-branch-protection-rules)
1. [Tag-Based Release Model](#3-tag-based-release-model)
1. [Role-Based Access Control](#4-role-based-access-control)
1. [Secure Publishing with OIDC (Trusted Publishing)](#5-secure-publishing-with-oidc-trusted-publishing)
1. [Validate Tag Author in CI](#6-validate-tag-author-in-ci)
1. [Prevent Invalid Release Tags](#7-prevent-invalid-release-tags)
1. [Full `publish.yml` with Governance Gates](#8-full-publishyml-with-governance-gates)
...
1. Save. No token required.

### GitHub Environment Setup

1. Go to **GitHub → Settings → Environments → New environment** → name it `release`
1. Add a protection rule: **Required reviewers** (optional but recommended for extra safety)
1. Add a deployment branch rule: **Only tags matching `v*`**
