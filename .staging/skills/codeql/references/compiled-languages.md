...
1. If multiple files at same depth, attempts to build all
1. Falls back to build scripts: `build.bat`, `build.cmd`, `build.exe`

**Linux/macOS autodetection:**

1. Look for build system in root directory
1. If not found, search subdirectories for unique build system
1. Run appropriate configure/build command
...
1. `MSBuild.exe` on solution/project files
1. Build scripts: `build.bat`, `build.cmd`, `build.exe`

**Linux/macOS autodetection:**

1. `dotnet build` on `.sln` or `.csproj` closest to root
1. `MSbuild` on solution/project files
1. Build scripts: `build`, `build.sh`
...
1. If none succeed, look for `go.mod` (`go get`), `Gopkg.toml` (`dep ensure -v`), or `glide.yaml` (`glide install`)
1. If no dependency managers found, rearrange directory for `GOPATH` and use `go get`
1. Extract all Go code (similar to `go build ./...`)
...
1. Run first found (Gradle preferred over Maven)
1. Otherwise, search for build scripts
