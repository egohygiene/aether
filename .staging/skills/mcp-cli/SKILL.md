...
1. **Explore**: `mcp-cli <server>` → see tools with parameters
1. **Inspect**: `mcp-cli <server>/<tool>` → get full JSON input schema
1. **Execute**: `mcp-cli <server>/<tool> '<json>'` → run with arguments
