...
1. Split each component's STRIDE table into Tier 1, Tier 2, Tier 3 sub-sections
1. Include `Exploitability Tier` and `Remediation Effort` on every finding — both are MANDATORY
1. STRIDE summary table MUST include T1, T2, T3 columns
...
1. `.md` files: start with `# Heading` on line 1. The `create_file` tool writes raw content — no code fences
1. `.mmd` files: start with `%%{init:` on line 1. Raw Mermaid source, no fences
1. Section MUST be titled exactly `## Action Summary`. Include `### Quick Wins` subsection with Tier 1 low-effort findings table
1. K8s sidecars: annotate host container with `<br/>+ Sidecar` — never create separate sidecar nodes (see `diagram-conventions.md` Rule 1)
1. Intra-pod localhost flows are implicit — do NOT draw them in diagrams
1. Action Summary IS the recommendations — no separate `### Key Recommendations` section
1. Include `> **Note on threat counts:**` blockquote in Executive Summary
1. Every finding MUST have CVSS 4.0 (score AND full vector string), CWE (with hyperlink), and OWASP (`:2025` suffix)
1. OWASP suffix is always `:2025` (e.g., `A01:2025 – Broken Access Control`)
1. Include Threat Coverage Verification table at end of `3-findings.md` mapping every threat → finding
1. Every component in `0.1-architecture.md` MUST appear in `2-stride-analysis.md`
1. First 3 scenarios in `0.1-architecture.md` MUST have Mermaid sequence diagrams
1. `0-assessment.md` MUST include `## Analysis Context & Assumptions` with `### Needs Verification` and `### Finding Overrides` tables
1. `### Quick Wins` subsection is REQUIRED under Action Summary (include heading with note if none)
1. ALL 7 sections in `0-assessment.md` are MANDATORY: Report Files, Executive Summary, Action Summary, Analysis Context & Assumptions, References Consulted, Report Metadata, Classification Reference
1. **Deployment Classification is BINDING.** In `0.1-architecture.md`, set `Deployment Classification` and fill the Component Exposure Table. If classification is `LOCALHOST_DESKTOP` or `LOCALHOST_SERVICE`: zero T1 findings, zero `Prerequisites = None`, zero `AV:N` for non-listener components. See `analysis-principles.md` Deployment Context table.
1. Finding IDs MUST be sequential top-to-bottom: FIND-01, FIND-02, FIND-03... Renumber after sorting
1. CWE MUST include hyperlink: `[CWE-306](https://cwe.mitre.org/data/definitions/306.html): Missing Authentication`
1. After STRIDE, run the Technology-Specific Security Checklist in `analysis-principles.md`. Every technology in the repo needs at least one finding or documented mitigation
1. CVSS `AV:L` or `PR:H` → finding CANNOT be Tier 1. Downgrade to T2/T3. See CVSS-to-Tier Consistency Check in `analysis-principles.md`
1. Use only `Low`/`Medium`/`High` effort labels. NEVER generate time estimates, sprint phases, or scheduling. See Prohibited Content in `output-formats.md`
1. References Consulted: use the exact two-subsection format from `output-formats.md` — `### Security Standards` (3-column table with full URLs) and `### Component Documentation` (3-column table with URLs)
1. Report Metadata: include ALL fields from `output-formats.md` template — Model, Analysis Started, Analysis Completed, Duration. Run `Get-Date -Format "yyyy-MM-dd HH:mm:ss" -AsUTC` at Step 1 and before writing `0-assessment.md`
1. `## Summary` table in `2-stride-analysis.md` MUST appear at the TOP, immediately after `## Exploitability Tiers`, BEFORE individual component sections
1. Related Threats: every threat ID MUST be a hyperlink to `2-stride-analysis.md#component-anchor`. Format: `[T02.S](2-stride-analysis.md#component-name)`
1. Diagram colors: copy classDef lines VERBATIM from `diagram-conventions.md`. Only allowed fills: `#6baed6` (process), `#fdae61` (external), `#74c476` (datastore). Only allowed strokes: `#2171b5`, `#d94701`, `#238b45`, `#e31a1c`. Use ONLY `%%{init: {'theme': 'base', 'themeVariables': { 'background': '#ffffff', 'primaryColor': '#ffffff', 'lineColor': '#666666' }}}%%` — no other themeVariables keys
1. Summary DFD: after creating `1.1-threatmodel.mmd`, run the POST-DFD GATE in Step 4. The gate and `skeleton-summary-dfd.md` control whether `1.2-threatmodel-summary.mmd` is generated.
1. Report Files table in `0-assessment.md`: list `0-assessment.md` (this document) as the FIRST row, followed by 0.1-architecture.md, 1-threatmodel.md, etc. Use the exact template from `output-formats.md`
1. `threat-inventory.json` MUST be generated for every analysis run (Step 8b). This file enables future comparisons. See `output-formats.md` for schema.
1. **NEVER delete, modify, or remove any existing `threat-model-*` or `threat-model-compare-*` folders** in the repository. Only write to your own timestamped output folder. Cleaning up temporary git worktrees you created is allowed; deleting other report folders is FORBIDDEN.
...
1. Mandatory Rules in `orchestrator.md` (this list)
1. Examples in `output-formats.md` (examples are illustrative, not authoritative when they differ from literal skeletons)
...
1. **Record start time & gather context**
   ...
   **Naming procedure (follow IN ORDER — stop at the first match):**
   ...
   1. **Primary script name** → `Import-Images.ps1` → `ImportImages`
   1. **Primary config/manifest name** → `Dockerfile` → `DockerContainer`, `values.yaml` → `HelmChart`
   1. **Directory name** (if component spans multiple files) → `src/ParquetParsing/` → `ParquetParser`
   1. **Technology name** (for external services/datastores) → "Azure OpenAI" → `AzureOpenAI`, "Redis" → `Redis`
   1. **External actor role** → `Operator`, `EndUser` (never drop these)
   ...
   **⛔ COMPONENT ELIGIBILITY — What qualifies as a threat model component:**
   ...
   1. **It is a top-level service**, not an internal helper (registered in DI, or the main entry point, or an agent with its own responsibility)
   1. **It would appear in a deployment diagram** — you could point to it and say "this runs here, talks to that"
   ...
   **Flow completeness checklist (use `<-->` bidirectional flows per the return flow rule above):**
   ...
   1. **Database/datastore flows**: `DF_Service_to_Redis` (bidirectional `<-->`). ONE flow per service-datastore pair.
   1. **Auth provider flows**: `DF_Service_to_AzureAD` (bidirectional `<-->`). ONE flow per service-auth pair.
   1. **Admin access flows**: `DF_Operator_to_Service` (bidirectional `<-->`). ONE per admin interaction.
   1. **Flow count locking**: After enumerating flows, LOCK the count. Two runs on the same code MUST produce the same number of flows (±3 acceptable). A difference of >5 flows indicates incomplete enumeration.
...
1. **Write architecture overview** (`0.1-architecture.md`)
...
1. **Inventory security infrastructure**
...
1. **Produce threat model DFD** (`1.1-threatmodel.mmd`, `1.2-threatmodel-summary.mmd`, `1-threatmodel.md`)
   ...
   1. Count boundaries (`subgraph` lines)
   1. If elements > 15 OR boundaries > 4:
   ...
   1. If threshold NOT met → skip summary, proceed to `1-threatmodel.md`
   1. Create `1-threatmodel.md` (include Summary View section if summary was generated)

1. **Enumerate threats** per element and flow using STRIDE-A (`2-stride-analysis.md`)
...
1. **For each threat:** cite files/functions/endpoints, propose mitigations, provide verification steps

1. **Verify findings** — confirm each finding against actual configuration before documenting
...
1. **Compile findings** (`3-findings.md`)
   ...
   1. CVSS prefix: every vector starts with `CVSS:4.0/` (NOT bare `AV:N/AC:L/...`)
   1. Related Threats: each threat ID is a separate hyperlink `[TNN.X](2-stride-analysis.md#anchor)` (NOT plain text)
   1. Sub-sections: `#### Description`, `#### Evidence`, `#### Remediation`, `#### Verification` (NOT `Recommendation`)
   1. Sort: within each tier → Critical → Important → Moderate → Low → higher CVSS first
   1. All 10 mandatory attribute rows present per finding
   1. **Deployment context gate (FAIL-CLOSED):** Read `0.1-architecture.md` Deployment Classification and Component Exposure Table.
   ...
   1. **If ANY threat has `⚠️ Accepted Risk`** → FAIL. The tool cannot accept risks. Go back and create a finding for each one.
   1. **If Platform ratio > 20%** → SUSPECT. Re-examine each `🔄 Mitigated by Platform` entry: is the mitigation truly from an EXTERNAL system managed by a DIFFERENT team? If the mitigation is the repo's own code (auth middleware, file permissions, TLS config, localhost binding), reclassify as `Open` and create a finding.
   1. **If ANY `Open` threat in `2-stride-analysis.md` has NO corresponding finding** → create a finding NOW. Use the threat's description as the finding title, the mitigation column as the remediation guidance, and assign severity based on STRIDE category.
   1. **Update `3-findings.md`** with the newly created findings. Renumber sequentially. Update the Coverage table to show `✅ Covered` for each.
   1. **This loop is the ENTIRE POINT of the Coverage table** — it's not documentation, it's a self-check that forces complete coverage. If you write the table and don't act on gaps, you've wasted the effort.
...
- **This is the #1 quality issue observed in testing.** Large repos (20+ components, 80+ threats) frequently have truncated JSON arrays because the model runs out of output tokens. If ANY array is truncated, you MUST rebuild it before proceeding. Do NOT finalize with mismatched counts.
   ...
   1. **DO NOT attempt to patch** the truncated file — partial JSON is unreliable
   1. **Regenerate using one of these strategies** (in preference order):
   ...
   1. **Re-validate** after regeneration — if still mismatched, repeat with the next strategy
   1. **NEVER proceed to Step 9 (assessment) or Step 10 (verification) with mismatched counts**
   ...
   1. **Append threats in batches** — write threats array with ~20 threats per append operation. Use `replace_string_in_file` to add batches to the existing file rather than writing the entire JSON in one `create_file` call.
   1. **Append findings** — similarly batch if >15 findings
   1. **Final validation** — read the completed file and verify all array lengths match metrics
...
1. **Write assessment** (`0-assessment.md`)
   ...
   1. `---` horizontal rules between EVERY pair of `##` sections (minimum 6)
   1. `### Quick Wins`, `### Needs Verification`, `### Finding Overrides` all present
   1. References: TWO subsections (`### Security Standards` + `### Component Documentation`) with 3-column tables and full URLs
   1. ALL metadata values wrapped in backticks; ALL fields present (Model, Analysis Started, Analysis Completed, Duration)
   1. Element/finding/threat counts match actual counts from other files
...
1. **Final verification** — iterative correction loop
    ...
    **Pass 1 — Comprehensive verification:**
    ...
    - If ANY check fails:
      ...
      1. Re-run ONLY the failed checks against the fixed file(s)
      1. Repeat until the failed checks pass
...
1. **Phase 2 — Diagram Rendering Checks**: Mermaid init blocks, classDef, styles, syntax
1. **Phase 3 — Cross-File Consistency Checks**: Component coverage, DF mapping, threat-to-finding traceability
1. **Phase 4 — Evidence Quality Checks**: Evidence concreteness, verify-before-flagging compliance
1. **Phase 5 — JSON Schema Validation**: Schema fields, array completeness, metrics consistency
1. **Phase 6 — Deterministic Identity**: Component ID stability, boundary naming, flow ID consistency
1. **Phase 7 — Evidence-Based Prerequisites**: Prerequisite deployment evidence, coverage completeness
1. **Phase 8 — Comparison HTML** (incremental only): HTML structure, change annotations, CSS
