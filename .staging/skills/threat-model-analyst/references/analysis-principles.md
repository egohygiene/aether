...
1. **Understand platform defaults** — research before assuming:
...
1. **Distinguish configuration states**:
...
1. **Finding density check:** As a guideline, expect roughly 1 finding per 2-3 significant components. If a repo has 15+ components and you have fewer than 8 findings, re-examine under-analyzed components.
1. **Use sub-agents for scale:** For repos with 10+ components, delegate component-specific STRIDE analysis to sub-agents to maintain depth. Each sub-agent should analyze 3-5 components.
1. **OWASP checklist sweep:** After component-level STRIDE, do a cross-cutting pass using the OWASP Top 10:2025 checklist below. This catches systemic issues (missing auth, no audit logging, no rate limiting, unsigned images) that component-level analysis may miss.
1. **Infrastructure-layer check:** Explicitly check for: container security contexts, network policies, resource limits, image signing, secrets management, backup/DR controls, and monitoring/alerting gaps.
1. **Exhaustive findings consolidation:** After STRIDE analysis is complete, scan the STRIDE output for ALL identified threats. Every threat MUST map to either:
...
1. **Minimum finding thresholds by repo size:**
...
1. **Context-aware Platform ratio limits (MANDATORY):**
...
1. **Authentication Check — Does the endpoint require credentials?**
...
1. **Authorization Check — What level of access is required?**
...
1. In Step 6/7 (finding verification), check each T1 candidate against the table above
1. If ANY override applies, downgrade to T2 (or T3 if multiple)
1. Document the override rationale in the finding’s Description
