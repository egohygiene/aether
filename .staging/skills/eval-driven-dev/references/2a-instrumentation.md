...
1. **Names must be unique** across the entire application (used as registry keys and dataset field names).
1. **Use `lower_snake_case`** for names.
1. **Don't change the function's interface** — `wrap()` is purely additive, returns the same type.
...
1. Every criterion that judges **what came out** has a corresponding `output` wrap.
1. Every criterion that judges **how the app decided** has a corresponding `state` wrap.
