...
1. Calls `Runnable.create()` to construct an instance, then `setup()` once
1. Runs all dataset entries **concurrently** (up to 4 in parallel):
   ...
   h. Runs evaluators
1. Calls `Runnable.teardown()` once
