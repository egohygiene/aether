...
1. **Detect the OS** — run `uname -s` (or check for Windows via `$env:OS` / `%OS%`) to determine macOS, Linux, or Windows
1. **Look up the LSP server** — read `references/lsp-servers.md` for known servers, install commands, and config snippets
1. **Ask scope** — use `ask_user` to ask whether the config should be user-level (`~/.copilot/lsp-config.json`) or repo-level (`lsp.json` at the repo root or `.github/lsp.json`)
1. **Install the server** — run the appropriate install command for the detected OS
1. **Write the config** — merge the new server entry into the chosen config file (`~/.copilot/lsp-config.json` for user-level; `lsp.json` or `.github/lsp.json` for repo-level). If a repo-level config already exists, keep using that location; otherwise ask the user which repo-level location they prefer. Create the file if missing and preserve existing entries.
1. **Verify** — confirm the LSP binary is on `$PATH` and the config file is valid JSON
...
1. Re-launch `copilot` in a project with files of the configured language
1. Run `/lsp` to check the server status
1. Try code intelligence features like go-to-definition or hover
