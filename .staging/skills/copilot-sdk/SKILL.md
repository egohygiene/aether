...
1. **Language runtime**: Node.js 18+, Python 3.8+, Go 1.21+, or .NET 8.0+
...
1. **Set timeouts**: Use `sendAndWait` with timeout for long operations
1. **Handle events**: Subscribe to error events for robust error handling
1. **Use streaming**: Enable streaming for better UX on long responses
1. **Persist sessions**: Use custom session IDs for multi-turn conversations
1. **Define clear tools**: Write descriptive tool names and descriptions
