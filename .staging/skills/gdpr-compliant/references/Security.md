...
1. Encrypt the DEK with a **Key Encryption Key (KEK)** stored in the KMS.
1. Store the encrypted DEK alongside the encrypted data.
1. Deleting the KEK = effective crypto-shredding of all data encrypted with it.
...
1. **Severity classification** — Low / Medium / High / Critical based on data sensitivity and volume.
1. **Containment** — Revoke compromised credentials; isolate affected systems; preserve evidence (do NOT delete logs).
1. **Assessment** — What data was exposed? How many subjects? What is the risk level?
1. **DPA notification** — Use the supervisory authority's online portal; include: nature of breach, categories and approximate number of data subjects, categories and approximate number of records, contact point, likely consequences, measures taken.
1. **Data subject notification** — If high risk: clear language, nature of breach, likely consequences, measures taken, DPO contact.
1. **Post-incident review** — Root cause analysis; corrective measures; update runbook.
