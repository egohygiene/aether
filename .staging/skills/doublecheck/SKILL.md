...
1. After the response, add a `Verification` section.
1. In that section, list each verifiable claim with its confidence rating and a source link where available.
...
1. **Run the search** using `web_search`. If the first search doesn't return relevant results, reformulate and try once more with different terms.

1. **Evaluate what you find:**
...
1. **Record the result** with the source URL. Always provide the URL even if you also summarize what the source says.
...
1. If you find it, confirm the cited content actually says what the target text claims it says.
1. If you cannot find it at all, flag it as FABRICATION RISK. Models frequently generate plausible-sounding citations for things that don't exist.
...
1. **Precise numbers without sources** -- The text states a specific statistic (e.g., "78% of companies...") without indicating where the number comes from. Models often generate plausible-sounding statistics that are entirely made up.

1. **Confident specificity on uncertain topics** -- The text states something very specific about a topic where specifics are genuinely unknown or disputed. Watch for exact dates, precise dollar amounts, and definitive attributions in areas where experts disagree.

1. **Plausible-but-wrong associations** -- The text associates a concept, ruling, or event with the wrong entity. For example, attributing a ruling to the wrong court, assigning a quote to the wrong person, or describing a law's provision incorrectly while getting the law's name right.

1. **Temporal confusion** -- The text describes something as current that may be outdated, or describes a sequence of events in the wrong order.

1. **Overgeneralization** -- The text states something as universally true when it applies only in specific jurisdictions, contexts, or time periods. Common in legal and regulatory content.

1. **Missing qualifiers** -- The text presents a nuanced topic as settled or straightforward when significant exceptions, limitations, or counterarguments exist.
