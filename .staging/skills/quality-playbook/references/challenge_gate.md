...
1. **Standalone** — pointed at a `quality/` directory from a prior run to challenge specific bugs. Example: `"Read quality/writeups/BUG-042.md and the source code it references. Run the challenge gate on this bug."`
...
1. Reads the source code at the cited file:line (fresh read, not from the writeup)
1. Runs both rounds as described above
1. Writes the verdict to `quality/challenge/BUG-NNN-challenge.md`
1. If the verdict is REJECTED, suggests removing the bug from BUGS.md and tdd-results.json
