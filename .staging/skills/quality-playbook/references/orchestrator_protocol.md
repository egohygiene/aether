...
1. Report the failure with specifics.
1. Suggest retrying in a fresh sub-agent — phase writes are preserved incrementally, so a retry can pick up where the previous attempt left off.
1. Never skip phases — each depends on prior output.
